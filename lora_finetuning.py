...
def main(hf_token, csv_path, model_name, max_steps, push_entire_model, push_lora_adapters):
    ...
    if push_entire_model.lower() == "true":
        ...
        model.push_to_hub_merged(str(model_name+"lora-full"), tokenizer, save_method="merged_16bit", token=hf_token)
    
    if push_lora_adapters.lower() == "true":
        model.save_pretrained_merged("model", tokenizer, save_method="lora")
        model.push_to_hub_merged(f"{model_name}-lora-updated", tokenizer, save_method="lora", token=hf_token)
